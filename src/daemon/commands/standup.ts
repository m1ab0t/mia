...
export function loadDispatchSummary(
  ...
  if (!existsSync(tracesDir)) return summary;

  // Collect candidate date strings spanning the window
  ...
  for (const date of dates) {
    if (!available.has(date)) continue;
    let content: string;
    try {
    ...
    for (const line of content.split('\n')) {
      ...
      if (!t) continue;
      try {
        const rec = JSON.parse(t) as TraceRecord;
