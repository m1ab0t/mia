/**
 * memory — mia memory [list|search|add|stats]
 *
 ...
 * makes the memory store visible and lets you add, search, and audit facts
 * manually — turning the silent memory system into a transparent, interactive
 * knowledge base.
 *
 * Usage:
...
export interface MemoryArgs {
  subcommand: 'list' | 'search' | 'add' | 'stats';
...
export function parseMemoryArgs(argv: string[]): MemoryArgs {
  ...
  return { subcommand, query, content, limit, all };
}
...
export async function handleMemoryCommand(argv: string[]): Promise<void> {
  ...
  // Lazy-import: avoids loading LanceDB/ONNX when the user runs other commands
  ...
  if (args.subcommand === 'list') {
    ...
    console.log('');
    entries.forEach((r, i) => renderEntry(r.content, r.timestamp, r.type, i));
    console.log('');
    ...
    console.log(
      ...
      `  ·  ${gray}--all${x}${dim} for other types${x}`,
    );
    console.log('');
    return;
  }
  ...
  if (args.subcommand === 'search') {
    ...
    console.log('');
    results.forEach((r, i) => renderEntry(r.content, r.timestamp, r.type, i));
    console.log('');

    const count = results.length;
    console.log(`  ${dim}${count} result${count !== 1 ? 's' : ''} for${x} ${cyan}${args.query}${x}`);
    console.log('');
    return;
  }
  ...
  if (args.subcommand === 'stats') {
    ...
    try {
      ...
      if (stats.totalMemories === 0) {
        ...
        const ORDERED_TYPES = ['fact', 'conversation', 'summary', 'context'];
        const allTypes = new Set([...ORDERED_TYPES, ...Object.keys(stats.byType)]);

        for (const type of allTypes) {
          const count = stats.byType[type] ?? 0;
          if (count === 0 && !ORDERED_TYPES.includes(type)) continue;
