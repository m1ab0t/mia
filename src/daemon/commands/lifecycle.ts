 ...
 */
function getDaemonStatus(): { pid: number; status: ReturnType<typeof readStatusFile> } | null {
  cleanStalePid();
  const pid = readPidFile();
  ...
  return { pid: pid!, status: readStatusFile() };
}
 ...
 */
export function requireDaemonRunning(): { pid: number; status: ReturnType<typeof readStatusFile> } | null {
 ...
 * sees.
 */
export function pingDaemon(): boolean {
  const pid = readPidFile();
