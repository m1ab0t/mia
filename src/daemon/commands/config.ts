...
function renderConfig(config: Record<string, unknown>): void {
  ...
  console.log(row('concurrency', String(c.maxConcurrency ?? 3)));
  console.log(row('timeout',     fmtMs(c.timeoutMs ?? 30 * 60 * 1000)));
  console.log(row('route',       String(c.defaultRoute ?? 'coding')));
  console.log(row('classifier',  `${dim}${c.classifierModel ?? 'claude-haiku-4-5'}${x}`));
