...
beforeEach(() => {
  vi.mocked(requireDaemonRunning).mockReturnValue(null as never);
  vi.mocked(isPidAlive).mockReturnValue(false);
  vi.mocked(readPidFile).mockReturnValue(null as never);
...
describe('handleP2PCommand — status', () => {
  ...
  it('shows "starting up" when status is null', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue({ pid: 1, status: null } as never);
    const out = captureOutput();
    await handleP2PCommand('status');
  ...
  it('shows "offline" when p2pKey is falsy', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus(null) as never);
    const out = captureOutput();
    await handleP2PCommand('status');
  ...
  it('shows "online" when p2pKey is set', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('aabbccdd1234') as never);
    const out = captureOutput();
    await handleP2PCommand('status');
  ...
  it('shows the p2p key when online', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('deadbeef1234') as never);
    const out = captureOutput();
    await handleP2PCommand('status');
  ...
  it('shows peer count when online', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('aabbcc', 3) as never);
    const out = captureOutput();
    await handleP2PCommand('status');
  ...
  it('shows "p2p" header label', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('aabbcc') as never);
    const out = captureOutput();
    await handleP2PCommand('status');
...
describe('handleP2PCommand — qr', () => {
  ...
  it('prints the p2p key in the qr header', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('aabbccddeeff') as never);
    const out = captureOutput();
    await handleP2PCommand('qr');
  ...
  it('calls qrcode.generate with the base64-encoded key', async () => {
    vi.mocked(requireDaemonRunning).mockReturnValue(fakeDaemonStatus('deadbeef') as never);
...
describe('handleP2PCommand — refresh', () => {
  ...
  it('restarts daemon when it is running', async () => {
    vi.mocked(readPidFile).mockReturnValue(99 as never);
