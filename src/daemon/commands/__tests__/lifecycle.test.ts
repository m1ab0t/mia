...
describe('requireDaemonRunning — daemon absent', () => {
  ...
  it('returns null when no pid file exists', () => {
    expect(requireDaemonRunning()).toBeNull();
  });

  it('prints a "not running" message when daemon is absent', () => {
    requireDaemonRunning();
...
describe('requireDaemonRunning — stale pid', () => {
  ...
  it('returns null for a stale (dead) pid', () => {
    expect(requireDaemonRunning()).toBeNull();
  });

  it('removes stale pid and status files', () => {
    requireDaemonRunning();
    expect(removePidFile).toHaveBeenCalled();
    expect(removeStatusFile).toHaveBeenCalled();
  });
});
...
describe('requireDaemonRunning — daemon alive', () => {
  ...
  it('returns pid when daemon is running', () => {
    vi.mocked(readPidFile).mockReturnValue(1234);
    vi.mocked(isProcessRunning).mockReturnValue(true);
    vi.mocked(readStatusFile).mockReturnValue(null);

    const result = requireDaemonRunning();
    expect(result).not.toBeNull();
    expect(result?.pid).toBe(1234);
  });

  it('returns status from readStatusFile when available', () => {
    vi.mocked(readPidFile).mockReturnValue(1234);
    vi.mocked(isProcessRunning).mockReturnValue(true);
    vi.mocked(readStatusFile).mockReturnValue(makeStatus() as never);
  ...
  it('returns status=null when no status file exists', () => {
    vi.mocked(readPidFile).mockReturnValue(1234);
    vi.mocked(isProcessRunning).mockReturnValue(true);
    vi.mocked(readStatusFile).mockReturnValue(null);
...
describe('handleStart — daemon not running', () => {
  let logSpy: ReturnType<typeof vi.spyOn>;
  ...
  it('logs "started" with the child pid', async () => {
    await handleStart();
...
describe('handleStop — force kill after timeout', () => {
  ...
  beforeEach(() => {
    vi.useFakeTimers();
    vi.mocked(readPidFile).mockReturnValue(5678);
...
describe('handleDaemonCommand — restart', () => {
  ...
  beforeEach(() => {
    // stop path: no pid → "not running"
    // start path after stop: no pid → spawn
    vi.mocked(readPidFile).mockReturnValue(null);
    vi.mocked(isProcessRunning).mockReturnValue(false);
