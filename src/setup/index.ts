...
import { readMiaConfig, writeMiaConfig, setModelConfig } from '../config/mia-config.js';
...
const OPENAI_MODELS = [
  { value: 'gpt-5.2-chat-latest', label: 'GPT-5.2 Chat Latest', hint: 'recommended · newest' },
  { value: 'gpt-5.2',             label: 'GPT-5.2',             hint: 'newest release' },
  { value: 'gpt-5.1-chat-latest', label: 'GPT-5.1 Chat Latest', hint: 'always latest 5.1' },
  { value: 'gpt-5.1',             label: 'GPT-5.1',             hint: 'latest chat' },
...
const GEMINI_MODELS = [
  ...
  { value: 'gemini-2.5-pro',         label: 'Gemini 2.5 Pro',   hint: 'highly capable' },
  { value: 'gemini-2.5-flash',       label: 'Gemini 2.5 Flash', hint: 'recommended · fast & capable' },
  { value: 'gemini-2.0-flash',       label: 'Gemini 2.0 Flash', hint: 'fast & efficient' },
];
...
export async function handleSetup(): Promise<void> {
  ...
  if (activePlugin === 'claude-code' && isTTY) {
    ...
    const claudeModel = await pickModel('Which model should Claude Code use for coding?', ANTHROPIC_MODELS, 'claude-sonnet-4-6');
    ...
    const codexModel = await pickModel('Which model should Codex use for coding?', OPENAI_MODELS, 'gpt-5.2-chat-latest');
    ...
    if (hasCredentials) {
      p.log.success('opencode credentials already configured');
    } else {
      const authMethod = await p.select({
    ...
    const opencodeModels = [
      ...
      { value: 'openai/gpt-5.2-chat-latest',   label: 'GPT-5.2 Chat Latest', hint: 'OpenAI · newest' },
      { value: 'openai/gpt-5.2',               label: 'GPT-5.2',            hint: 'OpenAI · newest release' },
    ...
    if (existingGeminiKey) {
      ...
      if (authMethod === 'oauth') {
        ...
        try {
          ...
          p.log.warn(`OAuth error: ${err instanceof Error ? err.message : String(err)}`);
    ...
    const geminiModel = await pickModel('Which model should Gemini use for coding?', GEMINI_MODELS, 'gemini-2.5-flash');
  ...
  // ─── 3. Mia chat model + API key ───────────────────────────────────
  //
  // The model mia uses for its own chat/personality matches the plugin's
  // provider — codex → OpenAI, claude-code → Anthropic, opencode → either.
  // We pick a sensible fast default and don't ask unless opencode (where
  // the user may have authenticated with either provider).

  let chatModelId: string;

  if (activePlugin === 'codex') {
    // Default to gpt-5-nano (cheapest/fastest for chat)
    chatModelId = 'gpt-5-nano';
    const chatProvider = getProviderForModel(chatModelId);
    setModelConfig('general', { model: chatModelId, provider: chatProvider });
    setModelConfig('coding', { model: chatModelId, provider: chatProvider });
    writeMiaConfig({ activeModel: 'general', generalModel: 'general', codingModel: 'coding' });
    p.log.success(`Chat model: ${chatModelId}`);

    // Ensure OPENAI_API_KEY is set (may already be set from auth step)
    const existing = getEnvVar('OPENAI_API_KEY');
    if (!existing && isTTY) {
      const key = await p.password({ message: 'OPENAI_API_KEY for mia chat (leave blank to skip)' });
      if (!p.isCancel(key) && key) {
        saveEnvVar('OPENAI_API_KEY', key as string);
        p.log.success('OPENAI_API_KEY saved · ~/.mia/.env');
      }
    }

  } else if (activePlugin === 'claude-code') {
    // Default to haiku — auth/key already handled above
    chatModelId = 'claude-haiku-4-5';
    const chatProvider = getProviderForModel(chatModelId);
    setModelConfig('general', { model: chatModelId, provider: chatProvider });
    setModelConfig('coding', { model: chatModelId, provider: chatProvider });
    writeMiaConfig({ activeModel: 'general', generalModel: 'general', codingModel: 'coding' });
  ...
  } else if (activePlugin === 'gemini') {
    // Default to gemini-2.5-flash — GEMINI_API_KEY already handled above
    ...
    // Ensure GEMINI_API_KEY is set (may already be set from auth step)
    const existingKey = getEnvVar('GEMINI_API_KEY');
    if (!existingKey && isTTY) {
      const key = await p.password({ message: 'GEMINI_API_KEY for mia chat (leave blank to skip)' });
      if (!p.isCancel(key) && key) {
        saveEnvVar('GEMINI_API_KEY', key as string);
        p.log.success('GEMINI_API_KEY saved · ~/.mia/.env');
      }
    ...
    // opencode — user may have either Anthropic or OpenAI, ask which to use for chat
    if (isTTY) {
      const chatModels = [
        { value: 'claude-haiku-4-5',    label: 'Claude Haiku 4.5',    hint: 'Anthropic · recommended · fastest' },
        { value: 'claude-sonnet-4-6',   label: 'Claude Sonnet 4.6',   hint: 'Anthropic · balanced' },
        { value: 'gpt-5-nano',          label: 'GPT-5 Nano',          hint: 'OpenAI · fastest & cheapest' },
        { value: 'gpt-5-mini',          label: 'GPT-5 Mini',          hint: 'OpenAI · fast & efficient' },
        { value: 'gpt-5.2-chat-latest', label: 'GPT-5.2 Chat Latest', hint: 'OpenAI · newest' },
      ];
      chatModelId = await pickModel('Which model should mia use for chat?', chatModels, 'claude-haiku-4-5');
    } else {
      chatModelId = 'claude-haiku-4-5';
    }
    const chatProvider = getProviderForModel(chatModelId);
    ...
    // Ensure the right key is set for whichever provider was chosen
    const isAnthropicChat = chatModelId.startsWith('claude');
    const envKey = isAnthropicChat ? 'ANTHROPIC_API_KEY' : 'OPENAI_API_KEY';
    const existing = getEnvVar(envKey);
    if (!existing && isTTY) {
      const key = await p.password({ message: `${envKey} for mia chat (leave blank to skip)` });
      if (!p.isCancel(key) && key) {
        if (isAnthropicChat) saveToken(key as string); else saveEnvVar(envKey, key as string);
        p.log.success(`${envKey} saved · ~/.mia/.env`);
      }
    }
  }
  ...
  if (isTTY) {
    const rerankerExists = existsSync(join(homedir(), '.mia', 'models', 'reranker.onnx'));
    const enableReranker = await p.confirm({
      message: rerankerExists
        ? 'Enable ONNX reranker for memory search? (slower but more accurate)'
        : 'Enable ONNX reranker? (model not downloaded — will use keyword fallback)',
      initialValue: false,
    });
    if (!p.isCancel(enableReranker)) {
      writeMiaConfig({ useReranker: !!enableReranker });
      p.log[enableReranker ? 'success' : 'info'](enableReranker ? 'Reranker enabled' : 'Reranker disabled · using keyword fallback');
    }
  }
