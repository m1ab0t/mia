...
describe('ContextPreparer.prepare() — mode routing', () => {
  it('general mode: skips memory, git, workspace, codebase context', async () => {
  ...
  it('coding mode: includes memory, git, workspace, and codebase context', async () => {
    ...
    expect(store.search).toHaveBeenCalledWith('fix the authentication bug', 10, true);
...
describe('ContextPreparer — memory facts', () => {
  ...
  it('returns empty array when memoryStore.search throws', async () => {
    const store = { search: vi.fn(async () => { throw new Error('LanceDB unavailable'); }) };
    const cp = new ContextPreparer({ workingDirectory: TEST_CWD, memoryStore: store });
    const ctx = await cp.prepare('run the tests', 'conv-1');
    expect(ctx.memoryFacts).toEqual([]);
  });

  it('passes useReranker=true by default', async () => {
    const store = makeMemoryStore([]);
    const cp = new ContextPreparer({ workingDirectory: TEST_CWD, memoryStore: store });
    await cp.prepare('fix the endpoint', 'conv-1');
    expect(store.search).toHaveBeenCalledWith(expect.any(String), 10, true);
  });

  it('passes useReranker=false when opted out', async () => {
    const store = makeMemoryStore([]);
    const cp = new ContextPreparer({
      workingDirectory: TEST_CWD,
      memoryStore: store,
    ...
    await cp.prepare('fix the endpoint', 'conv-1');
    expect(store.search).toHaveBeenCalledWith(expect.any(String), 10, false);
  });
});
...
describe('ContextPreparer — conversation summary', () => {
  ...
  it('filters out non-conversation message types', async () => {
    (getRecentMessages as Mock).mockResolvedValueOnce([
    ...
    expect(ctx.conversationSummary).not.toContain('Tool output');
  });
  ...
  it('truncates long message content to MESSAGE_PREVIEW_LENGTH with ellipsis', async () => {
    ...
    // Should truncate to MESSAGE_PREVIEW_LENGTH (300)
    const userLine = ctx.conversationSummary!.split('\n').find(l => l.startsWith('User:'))!;
    expect(userLine.length).toBeLessThan(350); // not the full 400
  });
...
describe('ContextPreparer — budget application', () => {
  ...
  it('truncates conversation summary when still over budget', async () => {
    ...
    if (ctx.conversationSummary) {
      // Summary was truncated if it contains the sentinel
      expect(ctx.conversationSummary).toContain('[truncated]');
      // Must be capped at SUMMARY_PREVIEW_LENGTH (500) + sentinel
      expect(ctx.conversationSummary.length).toBeLessThanOrEqual(520);
    }
  });
...
describe('ContextPreparer — adaptive budget tiers', () => {
  ...
  it('early tier: conversation summary cap is 500 chars', async () => {
    ...
    if (ctx.conversationSummary) {
      // Early summary cap: 500
      expect(ctx.conversationSummary.length).toBeLessThanOrEqual(500 + '[truncated]'.length + 5);
    }
  });

  it('extended tier: conversation summary cap is 200 chars', async () => {
    const longSummary = 'S'.repeat(800);
    ...
    if (ctx.conversationSummary) {
      // Extended summary cap: 200
      expect(ctx.conversationSummary.length).toBeLessThanOrEqual(200 + '[truncated]'.length + 5);
    }
  });
