...
describe('TraceLogger — recordEvent', () => {
  ...
  it('records events that appear in the flushed trace', () => {
    const tracesDir = join(tmpDir, 'traces');
...
describe('TraceLogger — endTrace', () => {
  it('writes an NDJSON file to the traces directory', () => {
    const tracesDir = join(tmpDir, 'traces');
    const logger = new TraceLogger({ tracesDir });
  ...
  it('written trace contains expected top-level fields', () => {
    const tracesDir = join(tmpDir, 'traces');
    const logger = new TraceLogger({ tracesDir });
  ...
  it('written trace contains the result', () => {
    const tracesDir = join(tmpDir, 'traces');
    const logger = new TraceLogger({ tracesDir });
...
describe('TraceLogger — retention cleanup', () => {
  it('deletes trace files older than retentionDays', () => {
    ...
    // Construction triggers cleanup
    new TraceLogger({ tracesDir, retentionDays: 7 });

    expect(existsSync(oldFile)).toBe(false);
  });

  it('keeps trace files within the retention window', () => {
  ...
  it('ignores non-.ndjson files during cleanup', () => {
  ...
  it('does not throw when traces directory does not exist yet', () => {
    const tracesDir = join(tmpDir, 'traces-nonexistent');
    // Do NOT create the directory; let the constructor handle it
    expect(() => new TraceLogger({ tracesDir, retentionDays: 7 })).not.toThrow();
  });
});
...
describe('TraceLogger — full start → record → end lifecycle', () => {
  it('produces a valid, parseable NDJSON trace with all recorded events', () => {
