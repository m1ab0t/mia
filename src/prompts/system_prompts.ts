...
• ~/.mia/mia.json        — main config: activePlugin, models, maxConcurrency, timeoutMs, pluginDispatch middleware settings, p2pSeed, classifierModel
• ~/.mia/PERSONALITY.md  — your personality and behavioral anchors (edit to change how you behave)
• ~/.mia/.env            — API keys (ANTHROPIC_API_KEY, BRAVE_API_KEY, etc.)
• ~/.mia/scheduled-tasks.json — cron task definitions
• ~/.mia/memory/         — daily markdown memory logs (YYYY-MM-DD.md)
• ~/.mia/memory.lance/   — vector memory store
...
export function buildCodingPrompt(mode: PromptMode = 'full'): string {
  ...
  const sections = [IDENTITY, DISPATCH_GUIDANCE];
