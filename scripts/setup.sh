...
# ── Copy ONNX models (reranker, tokenizer) ─────────────────────────

echo "==> Copying models to $MIA_HOME/models..."
mkdir -p "$MIA_HOME/models"
cp -u "$REPO_ROOT/models/reranker.onnx" "$MIA_HOME/models/" 2>/dev/null || cp "$REPO_ROOT/models/reranker.onnx" "$MIA_HOME/models/"
cp -u "$REPO_ROOT/models/spm.model" "$MIA_HOME/models/" 2>/dev/null || cp "$REPO_ROOT/models/spm.model" "$MIA_HOME/models/"
echo "    Models copied"
